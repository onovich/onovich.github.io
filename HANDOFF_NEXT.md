...
优先处理 tight gallery 顶部、graphic 长图尺寸和 main width 这些残差。Codes caption 已在 2026-06-10 切为 Cargo 风格 12px HTML caption；bodycopy / main content 行高也已同步为原站 `16px`，codes 5 断点列数 delta 为 0，wide root font-size 已修到 `15.55px`，pixel mobile 横向溢出已修，pixel 第二段 natural/flush gallery 的 `g2` 列数与 desktop/wide 图片宽高也已收敛，game/gif/illustrator mobile/tablet 已恢复 3 列。注意：旧 `main.y` 测的是右栏列盒，不等于首个可见内容起点；codes/pixel 标准 gallery 的 `mainAnchor.y` / `thumbnails.y` 5 断点 delta 已约为 0。
