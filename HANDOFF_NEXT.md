...
旧 Electron `admin/` 后台已移除，后续只沿站内 `/cms` 网页 CMS 演进。CMS 已拆出样式、浏览器 client、状态 helper、预览渲染和草稿校验；下一步优先拆导出/发布包构造。提交前仍要注意：`.claude/settings.local.json` 是本地设置变化，不要随手提交。
...
- P2：继续模块化网页 CMS：优先拆导出/发布包构造，再打磨富文本编辑
- P3：完善 CMS 发布包应用脚本、真实发布场景 smoke 和校验覆盖
