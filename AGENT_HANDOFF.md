...
### 进行中 / 待做
- 🟡 视觉残差消除（codes caption、bodycopy 行高、wide root font-size、home wide avatar、codes/pixel 标准 gallery 顶部、pixel mobile 横向溢出、pixel 第二段 natural/flush gallery、tight gallery 顶部与 game/gif/illustrator 断点列数、graphic 首张全栏图/长图顺序/laptop 2 列/mobile-tablet-laptop 图片宽度、Cargo grid 横向 gutter / main width 已收敛；顶部位置改看 `visual:measure` 的 `mainAnchor.y` / `thumbnails.y`，不要把旧 `main.y` 列盒位置当成首个内容起点；gif hero/natural media 尺寸继续逐项修）
