...
### 进行中 / 待做
- 🟡 视觉残差消除（codes caption、bodycopy 行高、wide root font-size、codes/pixel 标准 gallery 顶部、pixel mobile 横向溢出已收敛；顶部位置改看 `visual:measure` 的 `mainAnchor.y` / `thumbnails.y`，不要把旧 `main.y` 列盒位置当成首个内容起点；pixel 第二段 natural 缩略图尺寸/高度、tight gallery 顶部、game mobile/tablet 列数和 main width 继续逐项修）
