...
Production is served from Cloudflare Pages at `https://onovich.com`. The GitHub Pages user site remains available behind `https://blog.onovich.com` because project Pages from other repositories use that host as their shared namespace (for example, `/2DProGolf/`). Pages generated by this repository redirect that host to `onovich.com`; project subpaths remain on GitHub Pages. See [`../docs/CLOUDFLARE_PAGES_CUTOVER.md`](../docs/CLOUDFLARE_PAGES_CUTOVER.md).
