...
Production is served from Cloudflare Pages at `https://onovich.com`. The GitHub Pages project namespace is being moved from `https://blog.onovich.com` to `https://game.onovich.com` (for example, `/2DProGolf/`). Pages generated by this repository redirect either namespace host to `onovich.com`; during the migration, `blog.onovich.com` remains a compatibility entry point for existing project links. See [`../docs/CLOUDFLARE_PAGES_CUTOVER.md`](../docs/CLOUDFLARE_PAGES_CUTOVER.md).
