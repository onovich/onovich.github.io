...
As of 2026-08-26, `onovich.com` serves the bilingual portfolio through Cloudflare. `blog.onovich.com` still points at the GitHub Pages user site, which is also the shared path namespace for project sites such as `blog.onovich.com/2DProGolf/`. Because `blog` is semantically a poor name for a game-project namespace, the target is `game.onovich.com`; `blog` remains a compatibility host during the migration. The root portfolio copy is not intended to be a second production host: the host-aware redirect in `BaseLayout.astro` sends pages generated by this repository to `onovich.com` while leaving other repositories' project paths available.
...
- [ ] Add and verify the `game.onovich.com` DNS record and GitHub Pages custom-domain mapping.
- [ ] Verify the redirect bridge on both namespace roots and a representative set of project paths after the cutover.
...
The existing `site/public/CNAME` and GitHub Pages workflow must remain on `blog.onovich.com` until the new namespace is configured and verified. Do not disable the user Pages site, remove the current CNAME, or add a catch-all redirect before the new namespace is live: any of those can break paths such as `/2DProGolf/` and other repository pages.

The recommended migration sequence is:

1. Verify `onovich.com` at the GitHub account level, add `game.onovich.com` as the user-site custom domain, and create the DNS CNAME `game.onovich.com → onovich.github.io`.
2. Confirm that project Pages without their own CNAME resolve at `game.onovich.com/<repository>/`, including `2DProGolf` and `GameLetter`.
3. Change `site/public/CNAME` to `game.onovich.com` only after the Pages custom-domain setting and HTTPS certificate are ready.
