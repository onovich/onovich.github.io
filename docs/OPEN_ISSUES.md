...
3. P1：继续实测并复核 gallery 资源加载；`assets:check` 已确认 234 个真实内容图片引用 0 缺失，且没有超过 `1MB` 仍缺 `thumbSrc` 的候选；`visual:image-audit` 当前确认主要 gallery desktop clone 为 `113/113` 图片加载，mobile+desktop 扩展审计为 `226/226`。
...
- 已修：新增 `visual:image-audit` 无截图加载审计；本地 clone 默认主要 gallery desktop 审计为 `codes/game/pixel/illustrator/gif/graphic/photo` 共 `113/113` 图片加载，mobile+desktop 扩展审计为 `226/226`，避免为了判断加载状态反复生成截图。
...
**图片加载审计**：`site/scripts/visual-image-audit.mjs` 已加入，npm 脚本为 `npm run visual:image-audit`；用于无截图复核 clone/original 的 `images=loaded/total`，默认检查主要 gallery 页 desktop。
...
2. 缩略图列数和资源加载：先用 `visual:image-audit --clone=http://localhost:4350 --imageTimeout=25000 --scrollPasses=3` 复核主要 gallery；有 WARN 时再用 `visual:diff --targets=clone` 定向截图，之后决定是否需要页面级 eager / poster 调整。
